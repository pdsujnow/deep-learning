...
model = Doc2Vec.load('../doc2vec/sentiment140/sentiment140_lstm.d2v')
train_sents = []
train_labels = []
test_sents = []
test_labels = []
...
train_pos_count = 10000 #98912
train_neg_count = 10000 #99309
test_pos_count = 182
test_neg_count = 177

for i in range(train_pos_count):
    prefix_train_pos = 'TRAIN_POS_' + str(seq[i])
    train_sents.append(model.docvecs[prefix_train_pos].tolist())
    train_labels.append(1)

for i in range(train_neg_count):
    prefix_train_neg = 'TRAIN_NEG_' + str(seq[i])
